...
import { getAllLocations } from '@/api/geocoder'
...
export default Vue.extend({
  ...
  computed: {
    listViewHeight() {
      return !!this.locationsList && this.locationsList.length > 0
        ? this.locationsList.length * 64 + this.borderRadius
        : 0
    },
  ...
  methods: {
    ...
    async fetchLocationsList() {
      ...
      const searchLocationsResult: any = await getAllLocations(
        this.searchedLocation,
      )
      if (searchLocationsResult.length > 0)
        this.locationsList = searchLocationsResult
    },
    ...
    loadSearch() {
      ...
      const search = Utils.setInterval(() => {
        if (vm.searchedLocation !== vm.oldSearchedLocation) {
          ...
          if (vm.searchedLocation.length < vm.minimumCharactersToSearch)
            Utils.clearInterval(search)
        }
      }, vm.interval)
    },
